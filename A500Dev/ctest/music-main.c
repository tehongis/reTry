...
INCBIN_CHIP(SampleData,"Sample.raw")
...
static UBYTE AudPlayCount[] = { 0, 0, 0, 0 };

const BYTE SineData[] __attribute__((section (".MEMF_CHIP"))) = {
	0,64,111,127,111,64,
	0,-64,-111,-127,-111,-64
};
...
static __attribute__((interrupt)) void interruptHandlerLevel4() {
	const UWORD intreqr = custom->intreqr;
	if ( intreqr & INTF_AUD0 )
	{
		AudPlayCount[0]++;
		const UBYTE SampleIsRestarting = 2;
		if ( AudPlayCount[0] == SampleIsRestarting )
		{
			custom->dmacon=DMAF_AUD0;
		}
		custom->intreq=INTF_AUD0; custom->intreq=INTF_AUD0; //twice for a4000 bug.
	}
}
...
#if 0
	custom->aud[0].ac_ptr = (UWORD*)SineData;		// Pointer to the audio location (in chip memory)
	custom->aud[0].ac_len = 12 / 2;					// Length of the data to play, in words (not bytes)
	custom->aud[0].ac_vol = 64;						// Volume 0 -> 64, where 64 is max volume
	custom->aud[0].ac_per = 3546895 / (12 * 1000);	// Period???
	custom->dmacon = DMAF_SETCLR | DMAF_AUD0;		// Start the DMA for audio channel 0
#endif

	SetInterruptHandlerLevel4((APTR)interruptHandlerLevel4);
	custom->intena = INTF_SETCLR | INTF_AUD0;

	custom->aud[0].ac_ptr = (UWORD*)SampleData;		// Pointer to the audio location (in chip memory)
	custom->aud[0].ac_len = 88674 / 2;				// Length of the data to play, in words (not bytes)
	custom->aud[0].ac_vol = 64;						// Volume 0 -> 64, where 64 is max volume
	custom->aud[0].ac_per = 3546895 / 11025;		// Period???
	AudPlayCount[0] = 0;
	custom->dmacon = DMAF_SETCLR | DMAF_AUD0;		// Start the DMA for audio channel 0
